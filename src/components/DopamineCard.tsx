...
export default function DopamineCard({ record, streak, onUpdate }: DopamineCardProps) {
  ...
  return (
    <div className={`rounded-2xl p-5 transition-colors ${
      ...
      {/* Streak Display */}
      {displayStreak > 0 && (
        <div className="flex items-center gap-4 mb-4 p-3 rounded-xl bg-green-950/40 border border-green-900/30">
          <div className="relative flex-shrink-0">
            <div className="w-14 h-14 rounded-full bg-green-900/60 flex items-center justify-center border-2 border-green-500/50 shadow-[0_0_15px_rgba(34,197,94,0.3)]">
              <span className="text-2xl font-black text-green-400">{displayStreak}</span>
            </div>
          </div>
          <div>
            <p className="text-green-300 font-bold text-base">D+{displayStreak} 연속 성공</p>
            {streakMessage && (
      ...
      {displayStreak === 0 && !isDone && (
        <p className="text-gray-500 text-sm mb-4">오늘부터 연속 기록을 시작하세요</p>
      )}
